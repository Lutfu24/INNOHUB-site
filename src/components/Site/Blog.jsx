...
import { getBlogs, getBlogId } from "../../services/blogservices";
...
function Blog() {
  ...
  useEffect(() => {
    async function showBlogId(id) {
      const res = await getBlogId(id);
      setBlogId(res);
    }
    if (blogs) {
      showBlogId(blogs[0]._id);
    }
  }, [blogs]);
        ...
        >
          ...
          <div className="p-6 space-y-2 lg:col-span-5">
            ...
            <span className="text-sm dark:text-gray-600">
              {blogId?.createdAt.slice(0, 10)} {blogId?.createdAt.slice(11, 19)}
            </span>
            <p className="text-xl">{blogId?.content}</p>
          </div>
        </a>
