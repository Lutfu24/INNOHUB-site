...
import { Smartphone, Mail, Instagram, Linkedin, Facebook, Youtube, Music2 } from "lucide-react";

const API_URL = "https://api.innohub.alakbarova.site/api";
const ICONS = { Facebook, Linkedin, Instagram, Music2, Youtube, Smartphone, Mail };
...
export default function Footer() {
  ...
  const fallback = {
    ...
    contacts: [
      ...
      { icon: "Smartphone", title: "Mobil/Whatsapp", text: "+994 51 583 24 47" },
    ],
    links: [
      { title: "Tədris Sahələri", items: ["Dizayn", "Proqramlaşdırma", "Rəqəmsal Marketinq"] },
  ...
  return (
    <footer className="bg-gradient-to-t from-[#E0F8FF] to-white text-gray-800 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row md:justify-between items-center gap-6">
        <img src="https://i.ibb.co/kssj2vhX/dnisj.png" alt="Logo" className="h-12 md:h-16" />
        <div className="flex gap-3">{data.socials.map((s,i) => <Social key={i} {...s} />)}</div>
      </div>

      {/* Wave */}
      <svg viewBox="0 0 1440 180" className="w-full h-40" preserveAspectRatio="none">
        <path d="M0,64 C360,120 1080,0 1440,64 L1440,180 L0,180 Z" fill="#03C8FE" fillOpacity="0.05" />
      </svg>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-20 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="flex flex-col gap-4 md:col-span-2">
          {data.contacts.map((c,i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow hover:scale-[1.03] transition">
        ...
        {data.links.map((col,i) => (
          <div key={i}>
            ...
            <ul className="space-y-1 text-gray-600 text-sm">
              {col.items.map((l,j) => <li key={j} className="hover:text-[#03C8FE] cursor-pointer">{l}</li>)}
            </ul>
          </div>
        ))}
      </div>
